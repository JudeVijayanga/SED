...
lumin = open('gal_no_'+str(galId)+'_snap_'+str(snap)+'_fluxes.dat','w')
...
for band in cigaleBands:
    ...
    flux_band = extract_flux_in_band(wav.value, flux3, band_wave, band_trans)
...
#plt.show()
lumin.close()
fig.savefig('best_sed_%s_s%s.png'%(galId, snap))
...
fl_fir=open('gal_no_'+str(galId)+'_snap_'+str(snap)+'_physical_properties.dat','w')
